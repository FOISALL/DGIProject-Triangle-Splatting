...
def main():
    ...
    # Build numpy triangles and the same small test chunk inside this file.
    all_triangles = initialize_triangles(Globals.pointcloudData)
    chunk_indices = get_chunk_indices(Globals.pointcloudData, Globals.chunk_k)
    ...
    optimizer = optim.Adam(model.parameters(), lr=1e-4)  # Lowered from 1e-3 for stability
    ...
    while running:
        ...
        if train_mode and target_image is not None:
            ...
            if train_steps % 10 == 0:
                ...
                print(
                    f"Step {train_steps:4d} | avg loss {avg:.1f} | "
