...
def DrawRows(leftPixels: list[Pixel], rightPixels: list[Pixel], triangle: Triangle, vertexPixels: list[Pixel], surface: pygame.Surface):
    ...
    for row in range(len(leftPixels)):
        ...
        # improvemnt: we can precompute alpha here as a vectorized array
        alpha_span = triangle.opacity * influence

        T_span = Globals.transmittanceBuffer[y, x_start:x_end+1]

        # Front-to-back blending formula: C = C + color * alpha * T
        # We use [:, np.newaxis] to multiply (N,) alpha by (3,) color
...
while running:
    ...
    def get_depth(tri):
        # Calculate the centroid (arithmetic mean of vertices)
        centroid = np.mean(tri.vertices, axis=0)
    
        # Transform centroid to camera space: (World - Camera) * Rotation
        v_cam = (centroid - Globals.cameraPosition) @ Globals.R
    
        # Return the Z-depth (how far away it is along the camera's view axis)
        return v_cam[2]
    ...
    # 1. Clip and convert the accumulator to 8-bit integers
    # We transpose because Pygame expects (width, height, 3) and NumPy is (height, width, 3)
    final_image = np.clip(Globals.colorAccumulator, 0, 255).astype(np.uint8)
    final_image = np.transpose(final_image, (1, 0, 2))

    # 2. Fast blit to screen
