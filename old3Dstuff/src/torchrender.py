...
def compute_sdf_params(
    ...
    for i in range(3):
        ...
        dot = (n * (opp - a)).sum(-1)  # (N,)
        # We want dot > 0 (normal points toward opp); flip if not
...
def window_function(
    ...
    phi_s: float,       # scalar SDF at incenter
    sigma: float,       # softness exponent
) -> torch.Tensor:
    """
    ...
    """
    # pixel coords -> (P, 2)
    pts = torch.stack([px, py], dim=-1).float()
    ...
    ratio = (phi_p / phi_s).clamp(0.0, 1.0)
...
def render_differentiable(
    ...
    # We build the canvas as a list of additive contributions and multiply
    # by transmittance.  This must stay in the compute graph.
    ...
    for idx in sort_order:
        ...
        xs = torch.arange(x_min, x_max + 1, device=model.vertices.device, dtype=torch.float32)
        ys = torch.arange(y_min, y_max + 1, device=model.vertices.device, dtype=torch.float32)
        # meshgrid -> (rows, cols)
        ...
        influence = window_function(
            px, py,
            ns_all[idx], ds_all[idx],
            phi_s.item(), sigmas[idx].item(),
        )   # (P,)

        # alpha = opacity * influence
        alpha = opacities[idx] * influence   # (P,)  -- differentiable through opacities

        # The influence itself is not differentiable through sigma/phi_s via
        # .item() calls above; see note at the bottom of the file.
        # For a first working version this is fine; see TODO below.

        # Fetch transmittance for these pixels
        T = transmittance[y_min : y_max + 1, x_min : x_max + 1].reshape(-1)   # (P,)

        # Color contribution:  c * alpha * T
        color_contrib = colors[idx].unsqueeze(0) * (alpha * T).unsqueeze(-1) * 255.0   # (P, 3)

        # Write into canvas (scatter_add preserves the compute graph)
        flat_idx = (py.long() - y_min) * (x_max - x_min + 1) + (px.long() - x_min)

        # We need to work with a temporary 2-D patch to stay differentiable
        patch_canvas = canvas[y_min : y_max + 1, x_min : x_max + 1]  # view
        patch_flat = patch_canvas.reshape(-1, 3)
        # scatter_add_ is in-place but stays in the graph for the inputs
        patch_flat.scatter_add_(0, flat_idx.unsqueeze(-1).expand(-1, 3), color_contrib)

        # Update transmittance (in-place is fine here; T is not a leaf)
        T_patch = transmittance[y_min : y_max + 1, x_min : x_max + 1]
        T_patch_flat = T_patch.reshape(-1)
        new_T = T_patch_flat * (1.0 - alpha).detach()   # transmittance is a scheduling var, detach is correct
        transmittance[y_min : y_max + 1, x_min : x_max + 1] = new_T.reshape(y_max - y_min + 1, x_max - x_min + 1)
...
def training_step(
    ...
    """
    One gradient step.  Returns the scalar loss value.
    """
    optimizer.zero_grad()
...
def main():
    ...
    # Build numpy triangles (same init as before)
    from renderer import initialize_triangles, get_chunk_indices  # reuse your existing init
    all_triangles = initialize_triangles(Globals.pointcloudData)
    chunk_indices = get_chunk_indices.__wrapped__ if hasattr(get_chunk_indices, '__wrapped__') else None

    # fallback: inline chunk selection
    coords = np.array([[p.x, p.y, p.z] for p in Globals.pointcloudData])
    tree   = KDTree(coords)
    _, chunk_idx = tree.query(coords[0], k=Globals.chunk_k)
    chunk_indices = list(chunk_idx)
    ...
    # We'll use the FIRST rendered frame as the training target.
    # In a real pipeline you'd load a real photo here.
    target_image: torch.Tensor | None = None
    total_loss = 0.0
    train_steps = 0
    train_mode  = False   # press T to start training
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_t:
                    # Capture current render as training target and start training
                    train_mode = True
                    print("Training mode ON – current view is now the target image.")
        ...
        # ---- sort triangles (uses numpy vertices for speed) ----
        sync_model_to_triangles(model, all_triangles, chunk_indices)

        visible = [
            idx for idx in chunk_indices
            if get_depth_numpy(all_triangles[idx].vertices) > 0.1
        ]
        sorted_idx = sorted(visible, key=lambda i: get_depth_numpy(all_triangles[i].vertices))

        # ---- numpy preview render ----
        frame_np = numpy_render_to_array(all_triangles, sorted_idx, W, H)  # (H, W, 3) uint8

        # Capture target on first train-mode frame
        if train_mode and target_image is None:
            target_image = torch.from_numpy(frame_np.astype(np.float32)).to(device)
            print(f"Target captured: {target_image.shape}")

        # ---- training step (torch) ----
        if train_mode and target_image is not None:
            cam_t = torch.from_numpy(Globals.cameraPosition).to(device)
            R_t   = torch.from_numpy(Globals.R).to(device)

            # local sort order: indices into [0, len(chunk_indices))
            local_sorted = [chunk_indices.index(i) for i in sorted_idx if i in chunk_indices]

            loss_val = training_step(
                model, optimizer, target_image,
            ...
            if train_steps % 10 == 0:
                ...
                print(f"Step {train_steps:4d}  |  avg loss {avg:.1f}")

        # ---- blit to screen ----
        frame_transposed = np.transpose(frame_np, (1, 0, 2))   # pygame wants (W, H, 3)
        ...
        if Globals.show_debug:
            lines = [
                ...
                f"Triangles in chunk: {len(chunk_indices)}",
                f"Visible: {len(sorted_idx)}",
                f"Train mode: {'ON (T to toggle)' if train_mode else 'OFF – press T'}",
                f"Train steps: {train_steps}",
                f"Avg loss: {total_loss / max(1, train_steps):.2f}",
                "F1 toggle debug | WASD move | arrows rotate",
            ]
...
# ---------------------------------------------------------------------------
#
# 1. GRADIENT FLOW THROUGH SIGMA / PHI_S
#    In window_function() we call  phi_s.item()  and  sigmas[idx].item()
#    which detaches those values from the graph.  The influence tensor IS
#    computed from torch ops so alpha is still connected to opacities and
#    colors, but NOT to sigma or the SDF geometry yet.
#
#    To make sigma and phi_s differentiable too, replace:
#        influence = window_function(px, py, ns_all[idx], ds_all[idx],
#                                    phi_s.item(), sigmas[idx].item())
#    with a fully-torch version that keeps phi_s and sigmas[idx] as tensors:
#        ratio = (phi_p / phi_s).clamp(0.0, 1.0)   # phi_s still a tensor
#        influence = ratio.pow(sigmas[idx])          # sigmas[idx] still a tensor
#
...
#    full end-to-end gradients through T, remove the .detach() call.
#
# 4. PERFORMANCE
#    The Python loop over triangles is the bottleneck.  Once the math is
#    correct, move the per-pixel SDF evaluation to a batched operation or
#    a custom CUDA kernel (like the original 3DGS tile rasterizer).