...
CFG = {
    ...
    # Avoid infinite loops if many views stop producing a valid crop after the
    # model moves/densifies. If too many views in a row return crop=None, use
    # one central fallback crop so train_steps can keep advancing.
    "max_consecutive_crop_skips": 80,
    "fallback_to_central_crop_after_skips": True,
    ...
    "densify_every":  9600,
    "prune_every":    400,
    "max_triangles":  2200,   # hard cap; raise for fuller reconstruction
    "tau_prune":      0.005,

    # --- learning rates ---
    "lr_color_opacity_sigma": 3e-4,
    "lr_vertices_init":       5e-4,   # vertex LR at step 0
    "lr_vertices_final":      5e-5,   # vertex LR after decay_steps
    "lr_decay_steps":         8000,
    ...
    # --- saving ---
    "checkpoint_path":  "checkpoint_chunk_crop.pt",   # auto-saved every save_every steps
    "save_every":        100,              # checkpoint interval (steps)
    "save_image_every":  100,              # PNG export interval (steps)
    "image_dir":         "renders_chunk_crop",        # folder for PNG exports
    ...
    "training_render_dir": "renders_training_views",
...
def render_differentiable(model, cam_pos, R, focal, W, H,
    ...
    for idx in sort_order:
        ...
        alpha = opacities[idx] * ratio.pow(sigmas[idx])
...
def compute_model_crop(model, cam_pos, R, focal, W, H,
...
def compute_loss(canvas, target, model, lam=0.2, beta1=0.01, beta_area=1e-5, use_ssim=True):
    """
    ...
    total = photo + beta1 * lo + beta_area * area
...
def training_step(model, optimizer, scheduler, target,
    ...
    loss, components = compute_loss(canvas, target_for_loss, model, use_ssim=use_ssim)
        ...
        if train_mode and vis_views:
            ...
            if CFG.get("use_chunk_crop", True):
                crop = compute_model_crop(
                    model, cam_t, R_t, Globals.focalLength, W, H,
